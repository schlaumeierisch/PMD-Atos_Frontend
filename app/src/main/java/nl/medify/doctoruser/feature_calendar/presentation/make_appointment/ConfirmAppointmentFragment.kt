...
import nl.medify.doctoruser.databinding.FragmentConfirmAppointmentBinding
...
class ConfirmAppointmentFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val isTargetWithinTimeFrame = LocalDateTime.now().isBefore(LocalDateTime.of(LocalDate.now(), LocalTime.of(20, 25)))
        val selectedPatient = appointmentViewModel.selectedPatient

        selectedPatient?.addressDTO?.let { appointmentViewModel.getGeoCodes(it.city, it.street, it.zip) }
        ...
        binding.appointmentContainer.tvInfoPatient.text = selectedPatient?.let { "${it.firstName} ${it.lastName}" }
        binding.appointmentContainer.tvInfoLocation.text = selectedPatient?.addressDTO?.let { "${it.street}, ${it.zip}, ${it.city}" }
        ...
        binding.progressContainer.run {
            btnNext.setOnClickListener {
                appointmentViewModel.postCreateAppointment()
