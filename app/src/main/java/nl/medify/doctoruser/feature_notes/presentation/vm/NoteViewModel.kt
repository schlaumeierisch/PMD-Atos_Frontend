...
class NoteViewModel @Inject constructor(
    ...
    fun getNotesOfMedicalRecord(mrId: String, cpId: String) = viewModelScope.launch(Dispatchers.IO) {
        noteUseCases.getNotesOfMedicalRecordUseCase.invoke(mrId, cpId).collect {
            when(it) {
                is Resource.Success -> {
                    _noteList.value = NoteListState(noteList = it.data ?: emptyList())
                }
    ...
    fun postCreateNote(title: String, description: String, medicalRecordID: String) = viewModelScope.launch(Dispatchers.IO) {
        noteUseCases.postCreateNoteUseCase.invoke(title, description, medicalRecordID)
    }
}