...
import nl.medify.doctoruser.databinding.FragmentMedicationListBinding
...
class MedicationListFragment : Fragment() {
    ...
    private fun initViews() {
        ...
        // enable/disable the add medication button
        if (loginViewModel.loggedInUserRole == UserRole.GENERAL_PRACTITIONER) {
            binding.btnAddMedication.visibility = View.VISIBLE
        } else {
            binding.btnAddMedication.visibility = View.INVISIBLE
        }
        ...
        if (loginViewModel.loggedInUserRole == UserRole.GENERAL_PRACTITIONER) {
            medicationViewModel.getMedicationOfPatient(selectedPatient.medicalRecordId)
        } else {
            medicationViewModel.getMedicationOfMedicalRecord(
        ...
        viewLifecycleOwner.lifecycleScope.launch {
            viewLifecycleOwner.repeatOnLifecycle(Lifecycle.State.STARTED) {
                medicationViewModel.medicationList.collect {
                    medicationListAdapter = MedicationListAdapter(it.medicationList)
