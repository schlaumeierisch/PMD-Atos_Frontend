...
class MedicationViewModel @Inject constructor (
   private val medicationUseCases: MedicationUseCases
    ...
    fun getMedicationOfMedicalRecord(mrId: String, cpId: String) = viewModelScope.launch(Dispatchers.IO) {
        medicationUseCases.getMedicationOfMedicalRecordUseCase.invoke(mrId, cpId).collect {
            when(it) {
                is Resource.Success -> {
                    _medicationList.value = MedicationListState(medicationList = it.data ?: emptyList())
                }
    ...
    fun postCreateMedication(title: String, description: String, startDate: LocalDate, endDate: LocalDate, medicalRecordID: String) = viewModelScope.launch(Dispatchers.IO) {
        medicationUseCases.postCreateMedicationUseCase.invoke(title, description, startDate, endDate, medicalRecordID)
    }
}