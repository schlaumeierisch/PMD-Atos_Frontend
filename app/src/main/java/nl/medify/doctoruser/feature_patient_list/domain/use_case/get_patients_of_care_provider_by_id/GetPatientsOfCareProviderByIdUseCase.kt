package nl.medify.doctoruser.feature_patient_list.domain.use_case.get_patients_of_care_provider_by_id
...
class GetPatientsOfCareProviderByIdUseCase @Inject constructor(
    private val repository: PatientListRepository
) {
    operator fun invoke(id: String): Flow<Resource<List<Patient>>> = flow {
        try {
            emit(Resource.Loading())
            val patients = repository.getPatientsOfCareProviderById(id)
